...
VulkanSimpleRenderPass::VulkanSimpleRenderPass(VulkanContext* context, VulkanSwapChain* swapchain)
...
void VulkanSimpleRenderPass::cleanup()
{
...
VkExtent2D VulkanSimpleRenderPass::getPassExtent()
{
	return m_swapchain->getSwapChianExtent();
}

VulkanSimpleGraphicsPipeline::VulkanSimpleGraphicsPipeline(VulkanContext* context, VulkanRenderPass* pass, VkExtent2D extent)
	: VulkanGraphicsPipeline(context, pass, extent)
{
	createPipeline();
}

VulkanSimpleGraphicsPipeline::~VulkanSimpleGraphicsPipeline()
{
}

void VulkanSimpleGraphicsPipeline::createPipeline()
{
	auto vertShaderCode = VulkanHelper::readFile("Shader/vert.spv");
	auto fragShaderCode = VulkanHelper::readFile("Shader/frag.spv");
	...
	bindingDescription.stride = 0; // 无实际数据，仅用索引
	bindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

	// 定义顶点属性（空，仅用 gl_VertexIndex）
	std::vector<VkVertexInputAttributeDescription> attributeDescriptions; // 空列表
...
void VulkanSimpleGraphicsPipeline::cleanup()
{
	vkDestroyPipelineLayout(m_context->getDevice(), m_pipelineLayout, nullptr);
}

void VulkanSimpleGraphicsPipeline::beginRenderPass(VulkanCommandSystem* commandSystem, uint32_t imageIndex)
{
...
void VulkanSimpleGraphicsPipeline::endRenderPass(VulkanCommandSystem* commandSystem)
{
	vkCmdEndRenderPass(commandSystem->getPrimaryCommandBuffer());
}

void VulkanSimpleGraphicsPipeline::bindPipeline(VulkanCommandSystem* commandSystem)
{
	// 设置动态视口
	VkViewport viewport{};
	viewport.x = 0.0f;
...
void VulkanSimpleGraphicsPipeline::executeCommands(VulkanCommandSystem* commandSystem, const std::vector<std::unique_ptr<IRenderCommand>>& commands)
{
	vkCmdDraw(commandSystem->getPrimaryCommandBuffer(), 3, 1, 0, 0);
}
